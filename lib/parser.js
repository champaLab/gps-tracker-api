...
function parseHqData(raw) {
  ...
  try {
    ...
    // minimal shape: *HQ,DEVICE,VER,TIME,STATUS,LAT,NS,LON,EW,SPEED,COURSE,DATE,FLAGS,MCC,MNC,LAC,CELLID,IMEI,...
    if (parts.length < 12) {
    ...
    result.LAT_DDMM = parts[5];
    result.LAT_HEMI = parts[6];
    result.LON_DDMM = parts[7];
    result.LON_HEMI = parts[8];

    const lat = parseDdmmToDecimal(result.LAT_DDMM, result.LAT_HEMI);
    const lon = parseDdmmToDecimal(result.LON_DDMM, result.LON_HEMI);
    ...
    // Indexes are based on the sample you provided.
    if (parts[12]) result.FLAGS = parts[12];
    if (parts[13]) result.MCC = Number(parts[13]);
    if (parts[14]) result.MNC = Number(parts[14]);
    if (parts[15]) result.LAC = Number(parts[15]);
    if (parts[16]) result.CELLID = Number(parts[16]);
    if (parts[17]) result.IMEI = parts[17];
  } catch (err) {
    result.parseError = err.message;
  }
  return result;
